    ...
    if form.validate_on_submit():
        ...
        with app.app_context():
            ...
            if check_password_hash(password=password,
                                   pwhash=item.password):
                login_user(item)
                return redirect(url_for("get_all_posts"))
            else:
                flash("The password you entered is invalid. Try again.")
                return redirect(url_for("login"))
...
def show_post(post_id):
    ...
    requested_post = BlogPost.query.get(post_id)
    post_comments=requested_post.blog_comments
    # comment_author_id = requested_post.author_id
    # comment_author = User.query.get(comment_author_id)
